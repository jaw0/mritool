...
static void calc_means(Solid* means,  const Solid&  img, const Solid&  segm, int k, float winmin);
...
void Solid::segment(Solid& segm, int k,
	...
	int i, cdepth, cwidth, cheight;
	Vector energy(k);
	Solid *vals  = new Solid[k];
	...
	for(i=0;i<k;i++){
		...
		vals[i].finit(width, height, depth);
		pots[i].finit(width, height, depth);
		var[i].finit(width, height, depth);
	}
	
	for(float winsize=minwin; winsize<=1; winsize*=2){
		
		if(winsize!=minwin){
			cerr << "changing window size to " << winsize << endl;
			// update ...
		}else{
			// init
		...
		for(int iterno=0;iterno<maxit;iterno++){
			...
			for(z=0;z<cdepth;z++)for(y=0;y<cheight;y++)for(x=0;x<cwidth;x++){
...
static void calc_means(Solid* means, const Solid& img, const Solid& segm, int k, float winsize){
	Point p;
