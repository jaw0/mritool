    ...
    # $Id: Makefile,v 1.1 1995/11/23 04:30:50 jaw Exp jaw $


prefix = /usr/local

    # srcdir is used by make
srcdir = /home/sulcus/ur-cep/mritool-V2.7
...
ipl:
	cd iplib/lib ; make LIBDIR=$(solibdir)
...
jlisp.a: _jlispa
_jlispa:
	cd jlisp; $(MAKE) jlisp.a STANDALONE=-DNOT_STAND_ALONE version=$(jlversion) OPTIMIZE="$(OPTIMIZE)" \
		DEBUG="$(DEBUG)" CC=$(CC) CPP=$(CPP)
...
install: all help
	...
	-rm $(bindir)/mritool
	ln                 $(bindir)/$(TARGET) $(bindir)/mritool
	$(INSTALL) -m 555  jl        $(bindir)/jl
	$(INSTALL) -m 555  segmtoras $(etcdir)/segmtoras
	$(INSTALL) -m 555 etc/License  $(etcdir)/License
	$(INSTALL) -m 555 etc/help.txt $(etcdir)/help.txt
	cp lisp/*.jl $(lispdir)

clean:
	-rm -f $(LISPCODE) $(OBSLS) segmtoras xxjl jl jlisp.a $(TARGET) core a.out include/cli-defvar.list include/cli-defun.list include/defproto.h stamp configure.sed #*#
	cd jlisp;  $(MAKE) clean
	cd src; $(MAKE) clean
...
dist:
	./jl -f cep-dist
tar:
	cd $(jldir)/..; tar cf jlisp-$(jlversion).tar jlisp-$(jlversion)
	gzip --best ../jlisp-$(jlversion).tar

asnr:
	make -f Makefile.asnr
