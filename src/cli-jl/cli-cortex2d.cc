...
= "@(#)$Id: cli-cortex2d.cc,v 1.1 1995/11/23 04:30:09 jaw Exp jaw $";
#endif
...
// globals used to pass values in/out of our recursive functions
// so as to keep the stack from growing to much

static float ext_zdim, ext_thick, ext_theta, ext_ratio;
static Bimage *ext_image, *extr_image;
...
static void extr_counter(int x, int y){
	// count points in the yamakah
	// result is ext_count_[pt]
	...
	switch( l ){
	  ...
	  case 0:	// not visited, do y.test
...

		{
			...
			// find thickness
			float rad, thk;
			rad = sqrt( xxc*xxc + yyc*yyc );    	// radius to current pt
			thk = rad - ext_rad_h;
			if( thk < 0) thk = - thk;
...
static void extr_clear_region(int x, int y){
	// clear region in the flag image (extr_)
	// as it is faster to do it connectivity-wise
	// than a full clear
	int l;
  ...
  top:
	...
	switch( l ){
	  ...
	  case 1:
		extr_image->put(x,y, 0);
		break;
	}

	extr_clear_region(x+1, y);
	extr_clear_region(x-1, y);
	extr_clear_region(x, y+1);
	extr_clear_region(x, y-1);
}
	

static int extract_level_strct(int x, int y){
	// classify points by level and yamakah test...
  ...
  top:
	  ...
	  default:	// can't happen
		...
		{	//  see if it passes yamakah test
			...
			ext_rad_h = sqrt( ext_xc*ext_xc
				+ ext_yc*ext_yc );	// radius out to pt from center

			extr_counter(x, y);

			extr_clear_region(x,y);
			
			if( ext_count_p < ext_count_t * ext_ratio){
				// did not pass yamakah test
				ext_image->put(x,y, l?251:253);		// mark pt as failed
				return 0;
			}
			ext_image->put(x,y, l?252:254);			// mark pt as passed
		}
		break;
	}	// eo sw
	...
	// recurse
	extract_level_strct(x+1, y);
	extract_level_strct(x-1, y);
	extract_level_strct(x, y+1);
	extract_level_strct(x, y-1);
...
static void extract_stg2(Bimage &image, int x, int y, int pass){
	int l;
  ...
  top:
	...
	extract_stg2(image, x+1, y, pass);
	extract_stg2(image, x-1, y, pass);
	extract_stg2(image, x, y+1, pass);
	extract_stg2(image, x, y-1, pass);
}
	

DEFUN("yamakah2D", Fyamakah2d, Syamakah2d, 6,8,1,0,
      "(yamakah2D level1 level2 level3 thick theta squish [from to]) preliminary step in extracting cortex",
      (Obj l1, Obj l2, Obj l3, Obj thk, Obj tht, Obj squ, Obj from, Obj to))
{
	...
	if( !ui_jl || !ui_jl->c_image)
		return jlerror(Syamakah2d.name, IC_NIL, "no ui");

	if(! INUMP(l1))
		return jlerror(Syamakah2d.name, l1, "WTA: intp");
	if(! INUMP(l2))
		return jlerror(Syamakah2d.name, l2, "WTA: intp");
	if(! INUMP(l3))
		return jlerror(Syamakah2d.name, l3, "WTA: intp");
	if( !FLOATP(thk) && !INUMP(thk) && !DOUBLEP(thk))
		return jlerror(Syamakah2d.name, thk, "WTA: number p");
	if( !FLOATP(tht) && !INUMP(tht) && !DOUBLEP(tht))
		return jlerror(Syamakah2d.name, tht, "WTA: number p");
	if( !FLOATP(squ) && !INUMP(squ) && !DOUBLEP(squ))
		return jlerror(Syamakah2d.name, squ, "WTA: number p");
	...
	extr_image = new Uimage(solid.width, solid.height);
	for(int iy=0; iy<extr_image->height; iy++)
		for(int ix=0; ix<extr_image->width; ix++)
			extr_image->put(ix,iy,  0.0);

	for(int iz=0; iz< solid.depth; iz++){
		ext_image = &solid[iz];
		extract_level_strct(x,y);
	}
	...
	for(z=0;z<solid.depth; z++)
		extract_stg2(solid[z], solid.width /2, solid.height/2, 0);
	...
	for(z=0;z<solid.depth; z++)
		extract_stg2(solid[z], solid.width /2, solid.height/2, 1);
