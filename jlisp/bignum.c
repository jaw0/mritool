...
= "@(#)$Id: bignum.c,v 1.13 95/03/24 03:12:43 weisberg Exp Locker: weisberg $";
#endif
...
int prnbign(Obj a, Obj stream, int how){
	...
	Obj radix, vv=cint_to_big(1), foo, val=copy_big(a);
	Obj baz;
...
#if 0	
	foo = copy_big(val);
	while( Fgrequal(foo, MAKINT(base))==IC_TRUE){
		foo = big_short_div(baz=foo, base, '/');
		if( baz!=foo) free_big( baz );		/* try to keep garbage gen. in control... */
		vv  = big_short_multiply(baz=vv, base);
		if( baz!=vv) free_big( baz );
	}
#else
	/* a bit faster... */
