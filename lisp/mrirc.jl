...
(xdef mri:patdir    (mriname "/data/mri/crd")             stringp mriname "where the mri-scans are")
(xdef mri:segmdir   (mriname "/data/mri/segm")            stringp mriname "where the segm data is kept")
(xdef mri:etcdir    (mriname "/usr/local/lib/mritool/2.8/etc")             stringp mriname "where some goodies are")
